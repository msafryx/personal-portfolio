...
// ============================================================
// ✅ FIXED CAROUSEL (Credentials arrows + keyboard)
// ============================================================

// ===== Carousel State Management =====
const carouselState = {
    projects: { currentIndex: 0, itemsPerView: 1 },
    certifications: { currentIndex: 0, itemsPerView: 1 },
    credentials: { currentIndex: 0, itemsPerView: 1 }
};

// ID Map (your HTML IDs)
const carouselIdMap = {
    projects: { prev: 'projectsPrev', next: 'projectsNext', indicators: 'projectsIndicators' },
    certifications: { prev: 'certsPrev', next: 'certsNext', indicators: 'certsIndicators' },
    credentials: { prev: 'credsPrev', next: 'credsNext', indicators: 'credsIndicators' }
};
...
// ✅ Calculate itemsPerView from real widths (wrapper/card)
function recalcItemsPerView(type) {
    ...
    const perView = Math.max(1, Math.floor((wrapperWidth + gap) / (cardWidth + gap)));
    carouselState[type].itemsPerView = perView;
}

function updateItemsPerView() {
    recalcItemsPerView('projects');
    recalcItemsPerView('certifications');
    recalcItemsPerView('credentials');
}

// Clamp index after resize/data changes
function clampIndex(type, totalItems) {
...
window.addEventListener('resize', () => {
    ...
    if (typeof projectsData !== 'undefined') {
        clampIndex('projects', projectsData.length);
    ...
    if (typeof certificationsData !== 'undefined') {
        clampIndex('certifications', certificationsData.length);
    ...
    if (typeof credentialsData !== 'undefined') {
        clampIndex('credentials', credentialsData.length);
    ...
    carousel.addEventListener('touchend', (e) => {
        ...
        if (hasMoved) {
            ...
            if (Math.abs(diff) > 50) {
                e.preventDefault();
                e.stopPropagation();
                if (diff > 0) moveCarousel(type, 'next');
                else moveCarousel(type, 'prev');
            }
        }
        hasMoved = false;
    });
...
function updateCarouselIndicators(type, totalItems) {
    const ids = carouselIdMap[type];
    const indicators = document.getElementById(ids.indicators);
    if (!indicators) return;

    const state = carouselState[type];

    const totalPages = Math.max(1, Math.ceil(totalItems / state.itemsPerView));
...
function createCertificationCard(item, index, isCredential = false) {
    ...
    certCard.addEventListener('click', (e) => {
        if (e.target.closest('.cert-btn') || e.target.closest('a')) return;
        e.stopPropagation();
...
// ===== Keyboard Navigation for Carousels (FIXED) =====
document.addEventListener('keydown', (e) => {
    ...
    let type = null;
    if (isVisible(credSection)) type = 'credentials';
    else if (isVisible(certSection)) type = 'certifications';
    else return;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof projectsData !== 'undefined') renderProjects();
    if (typeof certificationsData !== 'undefined') renderCertifications();
    if (typeof credentialsData !== 'undefined') renderCredentials();
